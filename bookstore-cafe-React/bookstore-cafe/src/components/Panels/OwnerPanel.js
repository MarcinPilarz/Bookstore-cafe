...
const OwnerPanel = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/products?productType=${activeProductType}`)
      .then((response) => {
  ...
  const renderTableHeaders = () => {
    const commonHeaders = [
      <th key="name">Nazwa</th>,
      <th key="price">Cena</th>,
      <th key="description">Opis</th>,
    ];
    ...
    switch (activeProductType) {
      ...
      case "BOOK":
        return [
          ...commonHeaders,
          <th key="author">Autor</th>,
          <th key="genere">Gatunek</th>,
          <th key="publishingHouse">Wydawnictwo</th>,
          <th key="language">Język</th>,
          <th key="publicationDate">Data publikacji</th>,
          <th key="bookCover">Rodzaj okładki</th>,
          <th key="numberPage">Ilość stron</th>,
          <th key="numberBookStock">Ilość ksiązek w magazynie</th>,
        ];
      case "FOOD":
        return [
          ...commonHeaders,
          <th key="amountOfCalories">Kalorie</th>,
          <th key="foodWeight">Waga</th>,
        ];
      default:
  ...
  const handleImageUploadSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `http://localhost:8080/uploadImage?idProduct=${selectedProductId}`,
        formData,
        {
          headers: {
    ...
    return productPanel.map((product) => (
      <tr key={product.idProduct}>
        {editingProduct === product.idProduct ? (
          <>
            {renderEditableCell(
              editingProductData.productName,
              "productName",
              product.idProduct,
              "Nazwa produktu"
            )}
            {renderEditableCell(
              editingProductData.productPrice,
              "productPrice",
              product.idProduct,
              "Cena produktu"
            )}
            {renderEditableCell(
              editingProductData.productDescription,
              "productDescription",
              product.idProduct,
              "Opis produktu"
            )}
            ...
            {activeProductType === "BOOK" && (
              <>
                {renderEditableCell(
                  editingProductData.author,
                  "author",
                  product.idProduct,
                  "Autor"
                )}
                {renderEditableCell(
                  editingProductData.genere,
                  "genere",
                  product.idProduct,
                  "Rodzaj"
                )}
                {renderEditableCell(
                  editingProductData.publishingHouse,
                  "publishingHouse",
                  product.idProduct,
                  "Wydawnictwo"
                )}
                {renderEditableCell(
                  editingProductData.language,
                  "language",
                  product.idProduct,
                  "Język"
                )}
                {renderEditableCell(
                  editingProductData.publicationDate,
                  "publicationDate",
                  product.idProduct,
                  "Rok produkcji"
                )}
                {renderEditableCell(
                  editingProductData.bookCover,
                  "bookCover",
                  product.idProduct,
                  "Rodzaj okładki"
                )}
                {renderEditableCell(
                  editingProductData.numberPage,
                  "numberPage",
                  product.idProduct,
                  "Ilość stron",
                  "number"
                ...
                {renderEditableCell(
                  editingProductData.numberBookStock,
                  "numberBookStock",
                  product.idProduct,
                  "Ilość w magazynie"
                )}
            ...
            {activeProductType === "FOOD" && (
              <>
                {renderEditableCell(
                  editingProductData.amountOfCalories,
                  "amountOfCalories",
                  product.idProduct,
                  "Kalorie"
                )}
                {renderEditableCell(
                  editingProductData.foodWeight,
                  "foodWeight",
                  product.idProduct,
                  "Waga"
                )}
        ...
        <td>
          {editingProduct === product.idProduct ? (
            <button onClick={() => handleSaveClick(product.idProduct)}>
              Zapisz
            </button>
          ) : (
        ...
        <td>
          <button onClick={() => handleUploadImageClick(product.idProduct)}>
            Dodaj Zdjęcie
          </button>
        </td>
        <td>
        ...
        {activeTab === "produkty" && (
          <section className="product-container">
            ...
            <div className="product-tabs">
              <button onClick={() => setActiveProductType("COFFEE")}>
                Kawa
              </button>
              <button onClick={() => setActiveProductType("BOOK")}>
                Książki
              </button>
              <button onClick={() => setActiveProductType("FOOD")}>
                Jedzenie
              </button>
            </div>
